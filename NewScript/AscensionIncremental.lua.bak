            ...
            for key, current in pairs(ManagedLoops) do
                if current then
                    ...
                    if now >= (current.nextRunAt or 0) then
                        ...
                        if ManagedLoops[key] == current then
                            current.interval = NormalizeManagedLoopInterval(current.interval);
                            current.nextRunAt = os.clock() + current.interval;
                        end;
                    end;
                end;
            end;
...
-- Module tambahan (pastikan sudah ada di ReplicatedStorage)
local AscendEvent = ReplicatedStorage.Remotes.AscendEvent
...
-- =====================================================
-- AUTO UPGRADE BOARD (Toggle per Upgrade, Lock System)
-- =====================================================
-- =====================================================
-- AUTO UPGRADE BOARD (dengan Board Visibility seperti Client)
-- =====================================================

-- Module & Remote
local UpgradeConfig = require(ReplicatedStorage.Modules.UpgradeConfig)
local UpgradeRequestRemote = ReplicatedStorage.Remotes.UpgradeRequest
...
local Ascension = LocalPlayer:WaitForChild("AscensionStats"):WaitForChild("Ascension")
...
-- Gunakan ipairs agar terurut
for _, entryName in ipairs(sortedBaseKeys) do
    local entry = BaseUpgrade[entryName]
    local upgradeFrame = workspace.Upgrades[entryName]
    if upgradeFrame and upgradeFrame:FindFirstChild("Main") then
        local surfaceGui = upgradeFrame.Main:FindFirstChild("SurfaceGui")
        if surfaceGui then
            UpgradeTodo = surfaceGui.MainFrame.Upgrades.ScrollFrame
        end
    end
    Icon = nil
    if entry.StatName == "Diamonds" then
        Icon = "rbxassetid://98661491997407"
    else
        Icon = LocalPlayer.PlayerGui:WaitForChild("HUD", 10):WaitForChild("StatsDisplay"):WaitForChild("Content"):FindFirstChild(entry.StatName).ImageLabel.Image
    end
    -- UpgradeSection = MainTabs:Section({
    --     Title = string.format("<font color=\"%s\">%s Upgrade</font>", entry.Color, entry.StatName),
    --     Opened = true,
    --     TextXAlignment = "Center"
    -- })
    -- FM_Add("Board Upgrades", UpgradeSection)
    for _, Data in ipairs(UpgradeTodo:GetChildren()) do
        if Data.ClassName ~= "Frame" then continue end
        ...
        local toggle = currentGroup:Toggle({
            ...
            Callback = function(val)
                Config.AutoUpgradeBoards[Data.Name] = val
                if val then
                    StartManagedLoop(Data.Name, 0.2,
                        function()
                            if Config.AutoUpgradeBoards[Data.Name] then
                                local data = UpgradeConfig.Upgrades[Data.Name]
                                if data then tryBuyUpgrade(Data.Name, data) end
                            end
                        end
                    )
                else
                    StopManagedLoop(Data.Name)
                end
            end
        })
        BoardToggles[Data.Name] = toggle
        local boardData = {Name = Data.UpgradeNameText.Text, Image = Icon}
        DataBoardToggles[Data.Name] = boardData
        -- table.insert(DataBoards, boardData)  -- opsional, jika DataBoards tetap diperlukan
        -- DataBoardToggles[Data.Name] = table.insert(DataBoards,{Name = Data.Name, Image = Icon})
...
local ConvertEvent = ReplicatedStorage.Remotes.ConvertEvent
...
local RollRuneEvent = ReplicatedStorage.Remotes.RollRuneEvent
local RuneData = require(ReplicatedStorage.Modules.RuneData)  -- pastikan sudah ada
...
local function createRuneToggle(runeName)
    ...
    local toggle = runeGroup:Toggle({
        ...
        Callback = function(val)
            if val then
                StartManagedLoop("AutoRune_" .. runeName, 0.2, function()
                    return true
                end, function()
...
Config.AutoClick = {
    Enabled = false,
    Interval = 0.01  -- kecepatan klik (detik), bisa diubah
}
local HudClickRemote = ReplicatedStorage.Remotes.HudClick
-- =====================================================
-- AUTO CLICK (Farm)
-- =====================================================
...
local TierRequest = ReplicatedStorage.Remotes.TierRequest
local EventTierData = require(ReplicatedStorage.Modules.EventTierData)  -- data tier
local EventStatsFolder = LocalPlayer:FindFirstChild("EventStatsFolder") -- tempat Click & Balloon
local Resets = LocalPlayer:WaitForChild("Resets")                       -- tempat EventTier
local EventTier = Resets:WaitForChild("EventTier", 15)                 -- nilai tier saat ini

Config.AutoTier = {
    Enabled = false,
    Interval = 1  -- cek setiap 1 detik (hindari spam)
}
...
local EventStatsFolder = LocalPlayer:FindFirstChild("EventStatsFolder")
local PurchaseSkill = ReplicatedStorage.Remotes.PurchaseSkill   -- remote beli skill
...
-- =====================================================
local UpgradeSwordRequest = ReplicatedStorage.Remotes.UpgradeSwordRequest
...
local BalloonRequest = ReplicatedStorage.Remotes.BalloonRequest
Config.AutoBalloon = {
    Enabled = false,
    Interval = 1
}
...
local UpgradeTreeConfig = require(ReplicatedStorage.Modules.UpgradeTreeConfig)
local UpgradeTreeRequest = ReplicatedStorage.Remotes.UpgradeTreeRequest
...
local SacrificeRequest = ReplicatedStorage.Remotes.SacrificeRequest
local SPMulti = LocalPlayer:WaitForChild("Multis"):WaitForChild("SPMulti")
Config.AutoSacrifice = {
    Enabled = false,
    Interval = 1
}
...
local SandRequest = ReplicatedStorage.Remotes.SandRequest
...
local ElementalRankRequest = ReplicatedStorage.Remotes.ElementalRankRequest
local ElementalRank = Resets:WaitForChild("ElementalRank")  -- IntValue rank saat ini
Config.AutoElementalRank = {
    Enabled = false,
    Interval = 1
}
...
local FireClick = ReplicatedStorage.Remotes.FireClick
...
Config.AutoFireClick = {
    Enabled = false,
    Interval = 0.5  -- klik tiap 0.1 detik
}
...
-- YUHU
local AltarRoll = ReplicatedStorage.Remotes.AltarRoll
local DepositCore = ReplicatedStorage.Remotes.DepositCore
...
Config.AutoAltarRoll = {
    Enabled = false,
    Interval = 1    -- jeda antar roll (detik), karena ada animasi 2.4 detik
}
Config.AutoDepositCore = {
    Enabled = false,
    Interval = 5,
    Mode = "Stats"  -- "Stats", "Runes", atau "Both"
}
...
local ForgeDepositRequest = ReplicatedStorage.Remotes.ForgeDepositRequest
...
Config.AutoForgeDeposit = {
    Enabled = false,
    Interval = 3  -- deposit setiap 3 detik
}
...
local CollectCactus = ReplicatedStorage.Remotes.CollectCactus
...
Config.AutoCollectCactus = {
    Enabled = false,
    Interval = 0.5,      -- jeda untuk cek kaktus baru
    TweenTime = 0.3,     -- durasi tween ke kaktus (detik)
    MaxDistance = 100    -- hanya kaktus dalam radius ini yang diambil
}
-- =====================================================
