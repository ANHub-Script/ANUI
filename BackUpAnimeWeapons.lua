		...
		if type(v) == "table" then
			...
			if config and alive == true and uid and dataSection and dataSection.CFrame then
				...
				if not seenForUI[realName] then
                    ...
                    -- [BARU: Ambil Gambar dari ChanceReward]
                    local rewardImages = {}
                    if config.ChanceReward then
                        for rewardKey, _ in pairs(config.ChanceReward) do
                            local img = GetRewardAssetId(rewardKey)
                            if img then
                                table.insert(rewardImages, img)
                            end
                        end
                    end
					...
					table.insert(uiList, {
                        ...
                        Images = rewardImages -- Field Images ditambahkan untuk Dropdown
					});
				end;
			end;
		end;
...
local function GetAvatarsWithStats()
	...
	local MZFolder = ConfigsFolder:FindFirstChild("Multiple Zones") or ConfigsFolder:FindFirstChild("MultipleZones")
					...
					for _, enemyList in pairs(GlobalEnemyMap) do
						for _, enemyObj in ipairs(enemyList) do
							if enemyObj.Alive == true and enemyObj.Data and enemyObj.Data.CFrame then
								...
								local isBoss = config and config.Difficult == "Boss"
...
AvatarDropdown = GeneralManagerSection:Dropdown({
	...
	Callback = function(selectedItem)
		...
		-- Guard: Prevent None/nil/empty string
		if not avatarKey or avatarKey == "" or avatarKey == "None" then
			return;
		end;
		
		-- Update preview only if different and window open
