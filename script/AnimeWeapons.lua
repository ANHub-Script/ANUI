...
EnergyUpgradeConfig = require(ConfigsPath.BasicUpgrades.EnergyUpgrade)
UnlocksPrices = require(ConfigsPath.General.Unlocks)
YenModule = require(ConfigsPath.Machines.YenUpgrades)
YenUpgrade2Module = require(ConfigsPath.Machines.YenUpgrades2)
YenUpgrade2ToggleUI = {}
...
AvatarCursesConfig = require(AvatarCursesPath)
...
function CheckIsFightingZone()
    ...
    local fightingKeywords = {"Dungeon", "Raid", "Defense", "SorcerersDefense", "PirateTower", "ShadowGate", "CapitalRaid", "MagicRaid", "SkullRaid", "IgnitionRaid"} 
...
local function LoadSingleGamemodeData(configModule, displayFallback, valueName, maxKey, maxLabel)
    ...
    if success and module then
        ...
        local desc = maxLabel .. ": " .. maxValue .. " | HP Base: " .. UtilsModule.ToText(hpBase)
        local rewardCards = GenerateCardRewards(module.ChanceReward)

        return {{
            Title = module.Display or displayFallback,
            Value = valueName,
            Desc = desc,
            Images = rewardCards
        }}
...
pirateTowerList = LoadSingleGamemodeData(ReplicatedStorage.Scripts.Configs.Gamemodes.PirateTower, "Pirate Tower", "PirateTower", "MAX_FLOOR", "Max Floors")
shadowGateList = LoadSingleGamemodeData(ReplicatedStorage.Scripts.Configs.Gamemodes.ShadowGate, "Shadow Gate", "ShadowGate", "MAX_WAVE", "Max Waves")
capitalRaidList = LoadSingleGamemodeData(ReplicatedStorage.Scripts.Configs.Gamemodes.CapitalRaid, "Capital Raid", "CapitalRaid", "MAX_WAVE", "Max Waves")
magicRaidList = LoadSingleGamemodeData(ReplicatedStorage.Scripts.Configs.Gamemodes.MagicRaid, "Magic Raid", "MagicRaid", "MAX_WAVE", "Max Waves")
ignitionRaidList = LoadSingleGamemodeData(ReplicatedStorage.Scripts.Configs.Gamemodes.IgnitionRaid, "Ignition Raid", "IgnitionRaid", "MAX_WAVE", "Max Waves")
sorcerersDefenseList = LoadSingleGamemodeData(ReplicatedStorage.Scripts.Configs.Gamemodes.SorcerersDefense, "Sorcerers Defense", "SorcerersDefense", "MAX_WAVE", "Max Waves")
...
function BuildGameModesDropdownValues()
    ...
    local singleLists = {pirateTowerList, shadowGateList, sorcerersDefenseList, capitalRaidList, magicRaidList, ignitionRaidList}
...
GameModesDrop = FM_Add("GameModes", FarmTab:Dropdown({
    ...
    ImagePadding = 6,
    Callback = function(val)
        -- Tambahkan ignitionRaidT di baris variabel local
        local dungeonT, raidT, defenseT, pirateT, shadowT, sorcerersT, capitalRaidT, magicRaidT, ignitionRaidT = {}, {}, {}, {}, {}, {}, {}, {}, {}
        local singleTargetMap = {
            PirateTower = pirateT,
            ShadowGate = shadowT,
            SorcerersDefense = sorcerersT,
            CapitalRaid = capitalRaidT,
            MagicRaid = magicRaidT,
            IgnitionRaid = ignitionRaidT -- Tambahkan ini
        }
    ...
    while not Window.Destroyed do
        if Config.AutoLeave then
            ...
            if HudGamemode and HudGamemode.Visible then
                ...
                -- Letakkan di bawah pengecekan MagicRaid
                if not modeActive then
                    local ignitionUI = HudGamemode:FindFirstChild("IgnitionRaid")
                    if ignitionUI and ignitionUI.Visible then
                        local txt = ignitionUI.wave.amount.Text 
                        currentVal = tonumber(string.match(txt, "%d+")) or 0
                        targetLimit = Config.AutoLeave_IgnitionRaid
                        modeActive = true
                    end
                end
...
-- Pemisah visual agar user tahu ini adalah Tier 2
local Yen2Separator = FarmTab:Paragraph({
    Title = "--- Yen Upgrades Tier 2 ---",
})
FM_Add("Upgrade: Yen", Yen2Separator)

-- Membuat UI Toggle untuk Tier 2 di kategori yang sama
upgradeOrder2 = {"Damage", "Mastery", "Yen","Drop","Critical","Run Speed"} -- Sesuai urutan di config YenUpgrades2
CreateUpgradeSection(FarmTab, "Upgrade: Yen", YenUpgrade2Module.Config, "AutoYen2_", YenUpgrade2ToggleUI, upgradeOrder2)
...
function Optimizer.Update_Avatar_Logic(pData)
    ...
    if pData.Attributes and pData.Materials then
        ...
        local maxLevel = isAvatarSelected and AvatarLevelsModule.GetMaxLevel(currentAvatar) or 100
        local myTokens = pData.Materials[tokenKey] or 0
        ...
        if isAvatarSelected then
            if EnemiesConfig[currentAvatar] then
                avatarDisplayName = EnemiesConfig[currentAvatar].Display
            end
            ...
            local tokenDisplayName = MaterialsModule[tokenKey].Display or tokenKey
            ...
            if isMaxed then
                newDesc = string.format(
                    ...
                    "Total Buff: <font color='#00ff00'>+%s%% Damage</font>\n" ..
                    "%s%s %s", 
                    UtilsModule.ToText(currentBuff), tokenIcon,tokenDisplayName, UtilsModule.ToText(myTokens, 0)
                )
                ...
                newDesc = string.format(
                    ...
                    UtilsModule.ToText(nextBuff),
                    UtilsModule.ToText(cost, 0), tokenIcon,tokenDisplayName,
                    UtilsModule.ToText(myTokens, 0)
                )
    ...
    if Config.AutoAvatarLevelUp and isAvatarSelected then
        local currentLevel = pData.AvatarLevels[currentAvatar] or 0
        local maxLevel = AvatarLevelsModule.GetMaxLevel(currentAvatar)
        local tokenKey = AvatarLevelsModule.Token or "AvatarToken"
        ...
        if currentLevel < maxLevel then
            ...
            if (pData.Materials[tokenKey] or 0) >= cost then
                if Reliable then
                    pcall(function() Reliable:FireServer("Avatar Upgrade") end)
                    task.wait(1) -- Debounce agar tidak spam
                end
            end
        end
    end
...
function Optimizer.Update_EnergyUpgrade_Logic(pData)
    ...
    if not EnergyUpgradeConfig or not IsPremium then
    ...
    local maxLvl = EnergyUpgradeConfig.MAX
    local tokenKey = EnergyUpgradeConfig.TOKEN_NAME
    local displayName = EnergyUpgradeConfig.Display
    local imageId = EnergyUpgradeConfig.ImageId
    
    -- Ambil Kalkulasi dari Fungsi Module
    local cost = EnergyUpgradeConfig.GetEvolveCost(EnergyUpgradeConfig, currentLvl)
    local bonusText = EnergyUpgradeConfig.GetDisplayBonus(currentLvl)
...
function Optimizer.Update_AutoUnlock_Logic(pData)
    ...
    for itemName, cost in pairs(UnlocksPrices) do
        ...
        if currentYen >= cost then
            ...
            if pData.Unlocked[itemName] == nil or pData.Unlocked[itemName] == false then
                isLocked = true
            -- Cek di tabel Vault (khusus kategori Roll/Gacha)
            ...
            if isLocked then
                ...
                if Notify then 
                    Notify("Auto Unlock", "Unlocking Feature: " .. itemName, "shopping-cart") 
                end
                task.wait(1)
                break 
            end
...
function Optimizer.Update_YenUpgrade2_Logic(pData)
    ...
    if pData.YenUpgrades2 then
        ...
        Optimizer.GenericUpgradeLogic(
            YenUpgrade2Module.Config,   -- List Config Tier 2
            pData.YenUpgrades2,          -- Data Level Player Tier 2
            YenUpgrade2Module,           -- Module Tier 2 (untuk hitung cost/buff)
