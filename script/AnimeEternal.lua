...
function Optimizer.Update_KillAura_Logic(data)
    ...
    if Config.AutoKillAura then
        ...
        local nearbyMonsters = MouseModule.Get_Monsters_InRange()
        local args = nil
...
function Optimizer.Update_Farm_Logic(pData)
    ...
    -- 2. Refresh list musuh di Dropdown
    
    -- 1. PROTEKSI TRANSISI (Sangat Penting)
    -- Jika sedang loading masuk dungeon (IsTransitioning = true), stop farm sementara.
    -- Jika baru keluar dungeon, IsTransitioning sudah false, jadi farm langsung jalan.
    if IsTransitioning then 
        return 
    end

    -- 3. Cek apakah Auto Farm diaktifkan di UI
    ...
    if inDungeon then
        ...
        -- Cari ID Map saat ini untuk memfilter mana monster map asli dan mana monster dungeon
        ...
        if debrisMonsters then
            for _, monster in pairs(debrisMonsters:GetChildren()) do
                if monster:IsA("Model") and monster.PrimaryPart then
                    ...
                    -- Target monster yang BUKAN bagian dari database map normal
    ...
    elseif hasSelection then
        -- === LOGIKA MAP NORMAL (BERURUTAN) ===
        local debrisMonsters = workspace:FindFirstChild("Debris") and workspace.Debris:FindFirstChild("Monsters")
        
        -- Reset index jika melebihi jumlah pilihan
        ...
        if targetName and debrisMonsters then
            for _, monster in pairs(debrisMonsters:GetChildren()) do
                if monster:IsA("Model") and monster.PrimaryPart then
        ...
        -- Jika target urutan ini tidak ditemukan, pindah ke index berikutnya
...
FM_Add("Farm", GameModeGroups:Toggle({
    Title = "Auto Join (Search Room)",
    Desc = "Mencari room aktif di Dungeons_Shared.",
    Flag = "AutoJoin_Cfg",
