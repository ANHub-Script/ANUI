...
const tr=(lang:Lang)=>(key:keyof typeof UI)=>UI[key][lang]??UI[key].en

function nav(p:string,lang:Lang):DefaultTheme.NavItem[]{const t=tr(lang);return[
 {text:t('guide'),link:`${p}/guide/introduction`,activeMatch:`${p}/guide/`},
 {text:t('elements'),link:`${p}/elements/`,activeMatch:`${p}/elements/`},
 {text:t('features'),link:`${p}/features/`,activeMatch:`${p}/features/`},
 {text:t('examples'),link:`${p}/examples/`,activeMatch:`${p}/examples/`},
 {text:t('reference'),link:`${p}/api/`,activeMatch:`${p}/api/`},
 {text:VERSION,items:[{text:t('releaseNotes'),link:`${GITHUB}/releases`},{text:'GitHub',link:GITHUB},{text:'Discord',link:DISCORD},{text:'YouTube',link:YOUTUBE}]}
]}

function sidebar(p:string,lang:Lang):DefaultTheme.Sidebar{const t=tr(lang);return[
 {text:t('gettingStarted'),items:[
  {text:t('introduction'),link:`${p}/guide/introduction`},{text:t('installation'),link:`${p}/guide/installation`},{text:t('quickStart'),link:`${p}/guide/getting-started`},{text:t('windowConfig'),link:`${p}/guide/window-configuration`},{text:t('tabsSections'),link:`${p}/guide/tabs-and-sections`}
 ]},
 {text:t('elements'),items:[{text:t('elements'),link:`${p}/elements/`}]},
 {text:t('features'),items:[
  {text:t('overview'),link:`${p}/features/`},{text:t('notifications'),link:`${p}/features/notifications`},{text:t('dialogsPopups'),link:`${p}/features/dialogs-and-popups`},{text:t('configFlags'),link:`${p}/features/config-and-flags`},{text:t('keySystem'),link:`${p}/features/key-system`},{text:t('themes'),link:`${p}/features/themes`},{text:t('localization'),link:`${p}/features/localization`},{text:t('scheduler'),link:`${p}/features/scheduler`},{text:t('openButton'),link:`${p}/features/open-button`}
 ]},
 {text:t('examples'),items:[{text:t('examples'),link:`${p}/examples/`}]},
 {text:t('reference'),items:[{text:t('apiCheatsheet'),link:`${p}/api/`}]}
]}

const localeMeta={en:{label:'English',p:''},id:{label:'Bahasa Indonesia',p:'/id'},ja:{label:'日本語',p:'/ja'},ru:{label:'Русский',p:'/ru'},th:{label:'ไทย',p:'/th'},zh:{label:'简体中文',p:'/zh'},ko:{label:'한국어',p:'/ko'}} as const
const locales=Object.fromEntries(Object.entries(localeMeta).map(([code,meta])=>{const lang=code as Lang;const t=tr(lang);return[code,{label:meta.label,lang:code,themeConfig:{nav:nav(meta.p,lang),sidebar:sidebar(meta.p,lang),outline:{label:t('onThisPage')},editLink:{pattern:`${GITHUB}/edit/main/docs${meta.p}/:path`,text:t('editLink')},lastUpdated:{text:t('lastUpdated')},docFooter:{prev:t('prev'),next:t('next')},footer:{message:t('footerMsg'),copyright:t('footerCopyright')}}}]}))

export default defineConfig({title:'ANUI',description:'A modern, feature-rich UI library for Roblox script executors.',base:'/',cleanUrls:true,lastUpdated:true,themeConfig:{search:{provider:'local'},socialLinks:[{icon:'github',link:GITHUB}],locales,langMenuLabel:'Languages',sidebarMenuLabel:'Menu',returnToTopLabel:'Back to top'}})
